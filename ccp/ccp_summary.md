...
오답 : 된 리소스의 구성이 정책을 잘 준수하고 있는지 모니터링하고 평가하는 서비스입니다. 인프라
...
VPC 흐름 로그 - VPC의 네트워크 인터페이스를 오가는 IP 트래픽에 대한 정보를 캡처할 수 있도록 지원하는 기능입니다. 이 로그에는 출발지/목적지 IP 주소, 포트, 프로토콜, 트래픽 허용/거부 여부 등의 정보가 포함되어 네트워크 트래픽을 모니터링하고 보안 문제를 진단하는 데 사용됩니다. 트래픽 제어가 아니고 모니터링.
...
**비즈니스 인텔리전스(BI) 대시보드에 자연어 처리(NLP) 기술을 결합하여, 사용자가 평범한 문장으로 질문하면 시각적인 데이터(차트, 그래프 등)로 답변해주는 서비스**

C. Amazon QuickSight Q - 정답 ✅
Amazon QuickSight는 AWS의 대표적인 BI 서비스입니다. 여기에 Q 기능은 기계 학습을 기반으로 하는 자연어 쿼리 기능입니다. 사용자가 대시보드에 "지난달 서울 지역에서 가장 많이 팔린 제품 10개는 뭐야?"와 같이 자연어로 질문을 입력하면, QuickSight Q가 질문을 이해하고 그에 맞는 차트나 그래프를 자동으로 생성하여 보여줍니다.

A. Amazon Macie - 오답
Macie는 S3에 저장된 데이터 중 개인 식별 정보(PII)와 같은 민감한 데이터를 탐지하고 보호하는 보안 서비스입니다.

B. Amazon Rekognition - 오답
Rekognition은 이미지와 동영상을 분석하는 컴퓨터 비전 서비스입니다.

D. Amazon Lex - 오답
Lex는 음성이나 텍스트를 이용해 **대화형 인터페이스(챗봇)**를 구축하는 서비스입니다.

**인터넷 연결이 불안정하거나 불가능한 원격 위치에서 대규모 데이터를 수집하고, 나중에 AWS 클라우드로 전송하는 상황**

C. AWS Snow 제품군은 이러한 요구 사항에 완벽하게 부합하는 물리적 디바이스(device) 그룹입니다.

AWS Snowcone: 휴대하기 쉽고 작으며, 엣지 컴퓨팅(edge computing) 및 데이터 수집에 적합합니다.

AWS Snowball: 더 큰 규모의 데이터를 전송하는 데 사용됩니다.

AWS Snowmobile: 엑사바이트(exabytes)급의 매우 큰 데이터를 전송하기 위한 대형 트럭 형태의 서비스입니다.

이 제품군은 인터넷 연결이 없는 환경에서 데이터를 저장하고, 필요에 따라 기본적인 컴퓨팅 작업을 수행한 후, 물리적으로 AWS 데이터 센터로 배송하여 데이터를 클라우드에 업로드하는 방식으로 작동합니다

B. AWS Transfer 제품군: AWS Transfer Family는 FTP, FTPS, SFTP 프로토콜을 사용하여 Amazon S3 또는 Amazon EFS로 파일을 전송하는 완전 관리형 서비스입니다. 이는 인터넷 연결이 안정적인 환경을 전제로 합니다.

D. AWS 마이그레이션 허브: AWS Migration Hub는 AWS로 마이그레이션하는 애플리케이션의 진행 상황을 한곳에서 추적하고 관리하는 서비스입니다. 데이터를 수집하는 솔루션이 아닙니다.

B. Amazon SageMaker: SageMaker는 데이터 과학자와 개발자가 기계 학습 모델을 구축, 훈련 및 배포할 수 있는 완전 관리형 플랫폼입니다. 검색 서비스 자체가 아닌, ML 모델을 만드는 도구입니다.

C. Amazon Augmented AI(A2I): A2I는 ML 예측 결과를 사람이 검토하는 워크플로를 쉽게 구축하도록 돕는 서비스입니다. ML 모델의 정확도를 높이기 위한 보조 도구입니다.

D. Amazon Polly: Polly는 텍스트를 실제 사람의 음성처럼 자연스럽게 변환하는 텍스트-음성 변환(Text-to-Speech) 서비스입니다. 검색과는 관련이 없습니다.


**AWS CAF에서 클라우드 전환을 위한 네 가지 주요 단계**

Envision(구상): 이 단계는 클라우드 전환의 비전, 목표, 기대 성과를 정의합니다. 클라우드 도입이 비즈니스에 어떤 가치를 가져올지 계획하는 단계입니다.

Align: 이 단계는 비즈니스, 인력, 거버넌스, 플랫폼, 보안, 운영 등 여섯 가지 관점에서 현재 조직의 역량과 클라우드 목표 간의 격차를 파악합니다. 클라우드 준비 상태를 평가하고, 이해관계자들의 합의를 도출하는 단계입니다.

Launch: 파일럿 프로젝트를 시작하고 점진적으로 비즈니스 가치를 제공하는 단계입니다.

Scale: 성공적인 파일럿 프로젝트를 확장하고, 클라우드 투자의 이점을 극대화하는 단계입니다.

A. AWS CloudHSM: 하드웨어 보안 모듈(HSM)을 제공하여 암호화 키를 안전하게 관리하는 서비스입니다. 네트워크 연결과는 관련이 없습니다.

AWS 키 관리 서비스(KMS): KMS는 암호화 키를 생성하고 관리하는 서비스입니다. 이는 비밀 정보를 암호화하는 데 사용되는 핵심 도구이지만, 비밀 정보(secrets) 자체를 저장하는 서비스는 아닙니다. Secrets Manager는 KMS를 사용하여 비밀 정보를 암호화합니다. 즉, KMS는 Secrets Manager의 내부에서 사용되는 구성 요소입니다.

**규정 준수(compliance) 요구사항을 충족하기 위해 자체 암호화 키를 생성, 제어 및 관리할 수 있는 단일 테넌트(single-tenant) 솔루션**

C. AWS CloudHSM은 하드웨어 보안 모듈(HSM)을 제공하는 서비스입니다. HSM은 암호화 키를 안전하게 생성, 저장 및 관리하는 전용 물리적 장치입니다.

단일 테넌트: CloudHSM은 고객 전용의 HSM 인스턴스를 제공하므로, 다른 사용자와 리소스를 공유하지 않습니다. 이는 규정 준수 요구사항을 충족하는 데 매우 중요합니다.

규정 준수 지원: FIPS 140-2 Level 3과 같은 엄격한 보안 표준을 준수합니다.

자체 제어: 사용자가 직접 HSM의 키를 관리하고 통제할 수 있습니다.

오답 해설
A. AWS 키 관리 서비스(KMS): KMS는 암호화 키를 생성하고 관리하는 멀티 테넌트(multi-tenant) 서비스입니다. 즉, 여러 사용자가 동일한 물리적 하드웨어를 공유합니다. KMS는 규정 준수를 지원하지만, 키에 대한 완전한 단독 제어가 아닌 AWS가 관리하는 공유 환경이라는 점에서 문제의 요구사항과 맞지 않습니다.

B. AWS 인증서 관리자(ACM): SSL/TLS 인증서를 프로비저닝, 관리 및 배포하는 서비스입니다. 암호화 키 관리와는 직접적인 관련이 없습니다.

D. AWS 시스템 관리자: 여러 AWS 리소스에 대한 운영 데이터를 보고, 운영 작업을 자동화하는 서비스입니다. 보안 키 관리와는 무관합니다.

**PostgreSQL 데이터베이스를 AWS로 마이그레이션**
최소한의 관리 오버헤드, 자주 사용 안함

Amazon Aurora Serverless는 이러한 요구 사항에 가장 적합합니다.

최소한의 관리: Aurora Serverless는 데이터베이스 인스턴스를 직접 프로비저닝하고 관리할 필요가 없는 완전 관리형 서비스입니다. 스케일링, 패치, 백업 등의 관리 작업을 자동으로 처리합니다.

자주 사용하지 않는 워크로드: 애플리케이션의 실제 요구에 따라 자동으로 데이터베이스 용량을 확장하고 축소합니다. 데이터베이스가 유휴 상태일 때는 용량을 0으로 줄여 사용한 만큼만 비용을 지불하게 되므로, 자주 사용하지 않는 워크로드에 매우 비용 효율적입니다.

A. Amazon EC2의 PostgreSQL: EC2 인스턴스에 PostgreSQL을 직접 설치하는 방식입니다. 이 경우 사용자가 OS부터 데이터베이스 관리, 백업, 패치, 스케일링 등 모든 것을 직접 관리해야 하므로 관리 오버헤드가 가장 큽니다.

B. PostgreSQL용 Amazon RDS: RDS는 완전 관리형 서비스이므로 관리 오버헤드는 줄여주지만, Aurora Serverless처럼 워크로드에 따라 자동으로 용량을 0으로 축소하지는 않습니다. 데이터베이스 인스턴스를 항상 실행 상태로 유지해야 하므로, 자주 사용하지 않는 워크로드에는 비용 효율적이지 않습니다.

C. Amazon Aurora PostgreSQL 호환 에디션: Aurora의 일반 프로비저닝 방식입니다. RDS와 마찬가지로 관리 오버헤드가 적지만, 서버리스처럼 자동으로 용량을 축소하지는 않아 간헐적인 워크로드에 비효율적일 수 있습니다.

**EC2 인스턴스가 중지되거나 종료되면 데이터가 삭제되는 임시 스토리지 유형**

Amazon EC2 인스턴스 스토어는 EC2 인스턴스를 실행하는 물리적 호스트 컴퓨터에 연결된 임시 블록 스토리지입니다.

**분산 애플리케이션에서 텍스트 메시지와 이메일 메시지를 모두 보내는 데 사용되는 AWS 서비스**

Amazon Simple Notification Service(SNS)**는 애플리케이션, 사람 및 디바이스에 알림을 전송하는 완전 관리형 메시징 서비스

B. Amazon Simple Email Service(SES): SES는 이메일 전송에 특화된 서비스입니다. 텍스트 메시지(SMS) 전송 기능은 제공하지 않습니다.

C. Amazon CloudWatch 알림: CloudWatch 알림은 AWS 리소스 및 애플리케이션을 모니터링하고, 특정 지표에 따라 알림을 보내는 데 사용됩니다. 직접적인 메시징 서비스라기보다는 모니터링 경보를 보내는 데 초점을 맞춥니다.

D. Amazon Simple Queue Service(SQS): SQS는 메시지 큐 서비스로, 메시지를 임시로 저장하여 다른 서비스가 비동기적으로 처리할 수 있도록 돕습니다. 메시지 전송 기능보다는 메시지 보관 및 비동기 처리에 더 중점을 둡니다.

**ETL(추출, 변환 및 로드) 데이터에 특별히 사용되는 관리형 AWS 서비스는 무엇인가?**

AWS Glue는 서버리스 데이터 통합 서비스로, ETL 작업에 특화되어 있습니다.

서버리스: 별도의 서버를 프로비저닝하거나 관리할 필요가 없습니다.

ETL 기능: 다양한 데이터 소스에서 데이터를 추출하고, Python(PySpark) 또는 Scala 코드를 사용해 변환하며, 최종 목적지에 로드하는 작업을 쉽게 수행할 수 있습니다.

데이터 카탈로그: 데이터 스키마를 자동으로 추론하여 데이터 카탈로그를 생성하고 관리하므로, ETL 워크플로를 훨씬 효율적으로 만들 수 있습니다.

AWS Snowball Edge: 엣지 컴퓨팅 기능을 갖춘 물리적 디바이스로, 인터넷 연결이 불안정한 환경에서 대량의 데이터를 전송하는 데 사용됩니다. ETL 작업과는 직접적인 관련이 없습니다.

Amazon MQ: 애플리케이션의 여러 구성 요소가 서로 메시지를 주고받을 수 있도록 돕는 메시지 브로커 서비스입니다. 데이터를 장기간 저장하는 것과는 전혀 관련이 없는, 소프트웨어 간의 '우편 서비스' 같은 역할을 합니다.

AWS Amplify:

역할: 웹 및 모바일 앱의 백엔드(로그인, 데이터베이스 등)를 쉽게 구축하도록 돕는 개발 프레임워크 및 도구 모음입니다. iOS 앱 개발을 '쉽게' 만들어주는 부품(SDK)을 제공하지만, 이 부품을 조립하여 최종 앱으로 만드는 '공장(macOS 환경)' 역할은 하지 못합니다.

AWS 앱 실행자 (AWS App Runner):

역할: 컨테이너화된 웹 애플리케이션 및 API를 간단하게 배포하고 실행하는 서비스입니다. iOS와 같은 네이티브 모바일 앱이 아닌, 웹 기반 서비스를 위한 것입니다.

AWS Application Discovery Service는 이름 그대로 온프레미스 데이터 센터에서 실행 중인 서버, 애플리케이션, 그리고 그들 간의 종속성에 대한 정보를 자동으로 수집하고 **'검색'**해주는 서비스입니다.

IAM 자격 증명 보고서는 계정의 모든 IAM 사용자와 그들의 자격 증명 상태에 대한 상세 정보를 담은 CSV 형식의 보고서 파일

 AWS IoT Greengrass: 센서나 게이트웨이 같은 소형 엣지 디바이스에 AWS의 일부 기능을 확장하는 소프트웨어